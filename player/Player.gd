...
func _physics_process(delta):
	...
	if Input.get_joy_axis(0, 2) < -joystick_deadzone or Input.get_joy_axis(0, 2) > joystick_deadzone:
		rotation_degrees.y -= Input.get_joy_axis(0, 2) * 2.5
	if Input.get_joy_axis(0, 3) < -joystick_deadzone or Input.get_joy_axis(0, 3) > joystick_deadzone:
		$Camera.rotation_degrees.x = clamp($Camera.rotation_degrees.x - Input.get_joy_axis(0, 3) * 2.5, -90, 90)
...
func crouch_animation(button_pressed):
	if button_pressed:
		if not crouched:
			$CrouchTween.interpolate_property($MeshInstance, "mesh:mid_height", $MeshInstance.mesh.mid_height, 0.2, 0.2, Tween.TRANS_SINE, Tween.EASE_IN_OUT)
			$CrouchTween.interpolate_property($CollisionShape, "shape:height", $CollisionShape.shape.height, 0.2, 0.2, Tween.TRANS_SINE, Tween.EASE_IN_OUT)
			$CrouchTween.interpolate_property($Camera, "translation:y", $Camera.translation.y, 1.35, 0.2, Tween.TRANS_SINE, Tween.EASE_IN_OUT)
			$CrouchTween.start()
			crouched = true
	else:
		if crouched:
			...
			$CrouchTween.interpolate_property($Camera, "translation:y", $Camera.translation.y, 1.6, 0.2, Tween.TRANS_SINE, Tween.EASE_IN_OUT)
			$CrouchTween.start()
			crouched = false
